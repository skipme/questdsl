 ...
 */
        ...
        public LineType EvaluateLineType(string line, Dictionary<string, string> parserGroups)
        {
            ...
            Match m = Regex.Match(line, @"^\s*\\(.*)$|^\s*--\s*arg(\d+)\s*\$?(\w+)\s*$|^\s*(-{3,10})\s*$|^\s*(.*)\s*:\s*(.*)$|^\s*([^=>!<\+-]+)\s*(==|<|>|!=|>=|<=)\s*([^\s=>!<\+-]+)\s*$|^\s*([^\s=>!<\+\-\*/%]+)\s*(=|\+=|-=)\s*([^=>!<\+\-\*/%]+)\s*$|^(.*)\s*(\+\+|--)\s*$|^\s*-->\s*([\w\.\$]*)\s*(.+)*$|^\s*([^\s=>!<\+\-\*/%]+)\s*=\s*([^=>!<\+\-\*/%]+)\s*(\+|-|\*|/|%)\s*([^=>!<\+\-\*/%]+)\s*$|^\s*>\s*(\w+)\s*(.+)\s*$");
            if (m.Success)
            {
                ...
                if (m.Groups[9].Success)
                {
                    if (parserGroups != null)
                    {
                        parserGroups.Add("left", m.Groups[7].Value.Trim());
                        parserGroups.Add("cond", m.Groups[8].Value);
                        parserGroups.Add("right", m.Groups[9].Value.Trim());
                    }
